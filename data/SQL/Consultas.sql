...
-- RFC 7
SELECT to_date(fecha, 'DD/MM/YY') AS FECHA, TIPOPRODUCTO.NOMBRE, sum(univendidas) AS CANTIDAD_VENDIDA, sum (total)AS TOTAL_INGRESOS FROM FACTURA 
...
where idSucursal = 38 AND idTipoProducto = 12 AND to_date(fecha, 'DD/MM/YY') BETWEEN '31/10/18' AND '11/11/18'
group by to_date(fecha, 'DD/MM/YY'), TIPOPRODUCTO.NOMBRE ORDER BY TOTAL_INGRESOS DESC;

--RFC 8
SELECT * FROM (
SELECT CLIENTE.NOMBRE, IDCLIENTE, EXTRACT(month FROM factura.fecha) AS MES, COUNT(FACTURA.ID) AS COMPRAS
FROM CLIENTE INNER JOIN FACTURA ON CLIENTE.ID = FACTURA.IDCLIENTE group by cliente.nombre, idcliente, EXTRACT(month FROM factura.fecha)
)
WHERE COMPRAS >=2;
